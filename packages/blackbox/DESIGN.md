...
| `store-jsonfile.ts` | JSON ファイル実装 (CLI / ゲームサーバ、write-through)。**サブパス `@ludiars/blackbox/file` のみ** (node:fs 依存を root バレルに入れない) | node:fs |
...
| Ludellus | `ludellus.face_login_accept` | 顔照合スコア (派生値のみ) → accept/reject/manual_review | 曖昧域のみ LLM。accept 方向は auto 昇格禁止 (詳細: `spec/feature/blackbox-face-auth-boost.md`) |
